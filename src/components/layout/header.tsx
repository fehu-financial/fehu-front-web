...
export function Header({ className }: HeaderProps) {
	...
	return (
		<header className={cn(className, "flex flex-row w-full justify-between shadow-sm p-4 sm:px-6 lg:px-8")}>
						...
						<div className="mt-4 space-y-4">
							<div className="flex items-start">
								<div className="ml-3 flex-1">
									...
									<p className="mt-1 text-sm">You have a new transaction of $50.00 from Amazon.</p>
								</div>
							</div>
							<div className="flex items-start">
								<div className="ml-3 flex-1">
									<p className="text-sm font-medium">Budget limit reached</p>
									<p className="mt-1 text-sm">Your "Dining Out" budget has reached its limit.</p>
								</div>
							</div>
						</div>
					</PopoverContent>
				...
				<DropdownMenu>
					<DropdownMenuTrigger asChild>
						<Button variant="ghost" className="ml-4 p-2 flex items-center">
							...
							<span className="ml-2 text-md font-medium hidden sm:inline-block">Monkey D. Luffy</span>
