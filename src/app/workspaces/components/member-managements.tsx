...
import { type WorkspaceMember, WorkspaceMemberRole } from "@/core/types/workspace";
...
export function MemberManagement({
	...
	const availableToAdd = availableUsers.filter((user) => !members?.some((member) => member.id === user.id));
									...
									<CommandGroup className="max-h-[200px] overflow-auto">
										{filteredUsers.map((user) => (
											<CommandItem
												key={user.id}
												onSelect={() => {
													onAddMember({ ...user, role: WorkspaceMemberRole.VIEWER });
													setOpen(false);
													setSearchQuery("");
												}}
						...
						{members?.map((member) => (
							<div key={member.id} className="flex items-center justify-between p-2 rounded-md hover:bg-muted">
								...
								<div className="flex items-center gap-2">
									<Select value={member.role} onValueChange={(value) => onUpdateRole(member.id, value)}>
									...
									<Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onRemoveMember(member.id)}>
										<X className="h-4 w-4" />
