...
const columns: ColumnDef<Asset>[] = [
	{
		...
		cell: ({ row }) => (
			<div className="flex items-center space-x-2 min-w-52">
				<img src={row.original.image} alt={`${row.original.name} logo`} className="w-8 h-8 rounded-full" />
	...
	},
	{
		...
		cell: ({ row }) => (
			<div className="text-right">
				<Badge
					variant={row.original.allocation > 20 ? "destructive" : row.original.allocation > 10 ? "warning" : "default"}
				>
			...
			<TooltipProvider>
				<Tooltip>
					<TooltipTrigger asChild>
						<div className="text-right font-medium">{row.original.averagePrice.toFixed(2)}</div>
					</TooltipTrigger>
					<TooltipContent>
	...
	},
	{
		...
		cell: ({ row }) => (
			<div className="flex items-center justify-end space-x-2">
				<span className={`font-medium ${row.original.projectedGainLoss >= 0 ? "text-green-600" : "text-red-600"}`}>
